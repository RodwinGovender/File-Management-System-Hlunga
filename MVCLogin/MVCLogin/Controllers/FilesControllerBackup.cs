...
namespace MVCLogin.Controllers
{
    public class FilesController1 : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Reference,Attorney,ClaimentName,ClaimentSurname,ClaimentID")] Files files)
        {
            if (ModelState.IsValid)
            {
                using (OurDbContext db = new OurDbContext())
                {
                    var isUserAlreadyExists = db.File.Any(x => x.Reference == files.Reference);
                    if (isUserAlreadyExists)
                    {
                        ModelState.AddModelError("Reference", "Client with this Reference already exists");
                        return View(files);
                    }
                ...
                ModelState.Clear();
                ViewBag.Message = files.ClaimentID + " " + files.ClaimentSurname + " Successfully added.";
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Reference,Attorney,ClaimentName,ClaimentSurname,ClaimentID")] Files files)
        {
            if (ModelState.IsValid)
            {
